...
预计更新时间不定 按照优先级排序
1. 酷狗概念喜欢自动下载(跟随3.0发布)
2. docker-compose方便一键部署（跟随3.0发布)）           


## 更新参考

2.15.27
1. 增加导出已下载歌单列表设置方便3.0导入避免重复下载已下载的歌曲

2.15.26
1. 使用新的酷我歌曲信息接口 
2. 使用新的酷我获取封面接口（搜索没改下载获取歌词详细内容使用保证下载的歌曲都是有图片的）
3. 使用PC加密歌词接口 解决web有时获取不到问题

2.15.25
1. 修复酷我歌曲基础信息获取还是有概率失败问题以及返回信息少字段问题

2.15.24
1. 修复酷我歌曲详情错误（增加了10次请求重试）
2. 修复酷我flac失效
